...
__global__ void gpu_update_biases(float modifier, float *a,float *b, size_t m, size_t n) {
    ...
    size_t col = blockIdx.x * blockDim.x + threadIdx.x;
    float sum = 0;
    //printf("gpu_update_biases m = %li. n = %li, modifier = %.5f\n", m, n, modifier);
    if( col < 1 && row < m)
    {
     //   printf("threadid %i %i, row = %li, value before = %.5f\n", threadIdx.y, threadIdx.x, row, a[row]);
        for(size_t i = 0; i < n; i++)
        {
      //      printf("threadid %i %i, adding b[%li * %li + %li] = %.5f to sum %.5f\n", threadIdx.y, threadIdx.x,row, n, i, b[row * n + i], sum);
            sum += b[row * n + i];
       //     printf("threadid %i %i, sum = %.5f \n", threadIdx.y, threadIdx.x,sum);
        }
        //printf("threadid %i %i, row %li sum = %.5f\n",  threadIdx.y, threadIdx.x,row, sum);
        a[row] -= modifier * sum;
        //printf("threadid %i %i, row = %li, value after = %.5f\n", threadIdx.y, threadIdx.x, row, a[row]);
    }
}
...
int calculate_cost_and_derivative() {
    ...
    gpu_sigmoid_cost_derivative<<<dimGrid, dimBlock>>>( tail->device_Activated_Output, device_y, device_Cost_Derivative, tail->output_size, mini_batch_size );
...
sub calculate_loss {
   calculate_cost_and_derivative() and die;
}
