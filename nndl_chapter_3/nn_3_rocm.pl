...
use ML::ROCM qw(create_network print_network feedforward calculate_loss backprop update_weights SGD save_network);
...
SGD($training_data, $epochs, $batch_size, $learning_rate, test_data=>$test_data, debug => $debug, lambda => $lambda,
evaluation_data => $validation_data,
    monitor_training_cost => 1, monitor_evaluation_cost => 1,
    monitor_training_accuracy => 1, monitor_evaluation_accuracy => 1
);
save_network("rocm_network_1.json");
